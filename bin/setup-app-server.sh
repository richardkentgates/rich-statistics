...
#
# Provisions a new app.richstatistics.com server from scratch on
...
#
# Options
#   --domain   FQDN to serve (default: app.richstatistics.com)
...
echo "Temp copy:  /tmp/rsa-webhook-token.txt (delete after copying)"
cp "${TOKEN_FILE}" /tmp/rsa-webhook-token.txt
chmod 600 /tmp/rsa-webhook-token.txt
echo ""
echo "${BOLD}── APP_SERVER_SSH_KEY ──${RESET}"
echo "Written to: ${CI_KEY_FILE}"
echo "Temp copy:  /tmp/rsa-ssh-key.txt (delete after copying)"
cp "${CI_KEY_FILE}" /tmp/rsa-ssh-key.txt
chmod 600 /tmp/rsa-ssh-key.txt
echo ""
echo "${BOLD}${RED}IMPORTANT: Delete /tmp/rsa-webhook-token.txt and /tmp/rsa-ssh-key.txt after copying${RESET}"
echo "${BOLD}${RED}  rm -f /tmp/rsa-webhook-token.txt /tmp/rsa-ssh-key.txt${RESET}"
echo ""
echo "${BOLD}${GREEN}════════════════════════════════════════════════════════${RESET}"
echo ""
