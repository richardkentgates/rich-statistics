...
| `includes/class-pwa-download.php` | OTP generation for app auth |
...
- IP `<WP_TEST_SERVER_IP>`, user `richardkentgates`, SSH key `~/.ssh/id_rsa`
- WordPress root `/srv/www/wordpress`, plugin path `.../wp-content/plugins/rich-statistics/`
- Deploy: `rsync -az --delete --exclude='.git' -e "ssh -i ~/.ssh/id_rsa" ./ <SSH_USER>@<WP_TEST_SERVER_IP>:/srv/www/wordpress/wp-content/plugins/rich-statistics/`
