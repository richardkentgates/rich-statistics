...
class MultisiteTest extends WP_UnitTestCase {
	...
	public function test_new_blog_hook_uses_wp_initialize_site(): void {
		$cb = has_action( 'wp_initialize_site', [ RSA_DB::class, 'on_new_blog_event' ] );
	...
	public function test_table_names_follow_rsa_prefix_convention(): void {
		$expected_suffixes = [ 'events', 'sessions', 'clicks', 'heatmap' ];
	...
	public function test_events_table_has_correct_structure(): void {
		...
		$cols = $wpdb->get_results( "DESCRIBE $table" ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared

		$col_names = array_column( $cols, 'Field' );

		$required = [ 'id', 'session_id', 'page', 'created_at' ];
