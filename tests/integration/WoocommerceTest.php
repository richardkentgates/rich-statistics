...
class WoocommerceTest extends WP_UnitTestCase {
	...
	public function test_wc_session_id_uuid_has_correct_version_and_variant(): void {
		...
		$this->assertContains( $parts[3][0], [ '8', '9', 'a', 'b' ], 'UUID variant nibble must be 8, 9, a, or b' );
	}
	...
	public function test_wc_add_to_cart_inserts_event_via_insert_event(): void {
		...
		$before = $wpdb->get_var(
			$wpdb->prepare( "SELECT COUNT(*) FROM " . RSA_DB::wc_events_table() . " WHERE product_id = %d", 99 )
		);
		...
		$row = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT session_id, event_type, product_id, quantity FROM " . RSA_DB::wc_events_table() . " WHERE product_id = %d",
				99
			)
		);
		...
		$wpdb->delete( RSA_DB::wc_events_table(), [ 'product_id' => 99 ] );
		unset( $_POST['rsa_sid'] );
	}
	...
	public function test_wc_add_to_cart_ajax_inserts_event(): void {
		...
		$wpdb->delete( RSA_DB::wc_events_table(), [ 'product_id' => 77 ] );
		unset( $_POST['rsa_sid'] );
	}
}