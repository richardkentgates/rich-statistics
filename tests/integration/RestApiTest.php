...
class RestApiTest extends WP_UnitTestCase {
	...
	public function test_unauthenticated_request_returns_401(): void {
		...
		$this->assertContains( $response->get_status(), array( 401, 403 ), 'Expected 401 or 403 for unauthenticated request' );
	}
	...
	public function test_admin_can_access_overview(): void {
		...
		$response = static::$server->dispatch( $request );
		$this->assertTrue(
			in_array( $response->get_status(), array( 200, 403 ), true ),
	...
	public function test_clicks_row_shape_when_data_present(): void {
		...
		$response = static::$server->dispatch( $request );
		$wpdb->delete( $wpdb->prefix . 'rsa_clicks', array( 'session_id' => 'rest-api-test-session' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		if ( 200 !== $response->get_status() ) {
