...
class CLITest extends WP_UnitTestCase {
	...
	private function seed_event( array $overrides = [] ): int {
		global $wpdb;
		$sid = 'cli-test-' . uniqid();
		$defaults = [
			'session_id'      => $sid,
	...
	private function seed_session( string $sid = '', array $overrides = [] ): string {
		global $wpdb;
		$sid = $sid ?: 'cli-session-' . uniqid();
		$defaults = [
			'session_id'   => $sid,
			'pages_viewed' => 1,
			'entry_page'   => '/test/',
	...
	public function test_get_overview_counts_events_and_sessions(): void {
		...
		$this->seed_event( [ 'session_id' => $sid ] );
		$this->seed_event( [ 'session_id' => $sid ] );
		$this->seed_event( [ 'session_id' => $sid ] );
	...
	public function test_get_top_pages_returns_page_data(): void {
		$this->seed_event( [ 'page' => '/page-1/' ] );
		$this->seed_event( [ 'page' => '/page-1/' ] );
		$this->seed_event( [ 'page' => '/page-2/' ] );
	...
	public function test_get_audience_returns_os_browser_language(): void {
		$this->seed_event( [ 'os' => 'Windows', 'browser' => 'Firefox', 'language' => 'en-US' ] );
		$this->seed_event( [ 'os' => 'Mac', 'browser' => 'Safari', 'language' => 'de-DE' ] );
	...
	public function test_export_events_includes_all_columns(): void {
		...
		$expected_cols = [ 'session_id', 'page', 'referrer_domain', 'os', 'browser', 'created_at' ];
	...
	public function test_prune_removes_old_events(): void {
		...
		$this->seed_event( [ 'created_at' => $old_date, 'session_id' => 'old-event-001' ] );

		$et = RSA_DB::events_table();
		$before = (int) $wpdb->get_var( "SELECT COUNT(*) FROM `{$et}` WHERE session_id = 'old-event-001'" );
		$this->assertSame( 1, $before );

		$deleted = RSA_DB::prune_old_data( 90 );

		$after = (int) $wpdb->get_var( "SELECT COUNT(*) FROM `{$et}` WHERE session_id = 'old-event-001'" );
	...
	public function test_prune_respects_custom_retention_days(): void {
		...
		$this->seed_event( [ 'created_at' => $old_date, 'session_id' => 'recent-event-001' ] );

		$et = RSA_DB::events_table();

		RSA_DB::prune_old_data( 30 );

		$after = (int) $wpdb->get_var( "SELECT COUNT(*) FROM `{$et}` WHERE session_id = 'recent-event-001'" );
