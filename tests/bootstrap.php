...
$wp_tests_dir = $wp_tests_dir ? $wp_tests_dir : '/tmp/wordpress-tests-lib';
...
if ( is_dir( $wp_tests_dir ) ) {
	...
	/**
	 * Load WooCommerce if available — needs Action Scheduler and WordPress init
	 * to complete first. We hook at 'init' priority 1 to ensure all WP APIs are
	 * ready before loading WC.
	 */
	tests_add_filter(
		'init',
		function () {
...
if ( ! function_exists( 'current_time' ) ) {
	function current_time( string $type ) {
		return gmdate( $type === 'timestamp' ? 'U' : 'Y-m-d H:i:s' ); }
}
...
if ( ! function_exists( 'wp_send_json_error' ) ) {
	function wp_send_json_error( $data = null, int $status = 400 ): void {
		echo json_encode(
			array(
				'success' => false,
				'data'    => $data,
...
if ( ! function_exists( 'wp_schedule_single_event' ) ) {
	function wp_schedule_single_event( int $time, string $hook, array $args = array() ): bool {
		return true; }
}
...
if ( ! function_exists( 'wp_mail' ) ) {
	function wp_mail( $to, string $subject, string $message, array $headers = array(), array $attachments = array() ): bool {
		return true; }
}
if ( ! function_exists( 'wp_remote_post' ) ) {
	function wp_remote_post( string $url, array $args = array() ) {
		return new WP_Error( 'disabled', 'wp_remote_post stub' ); }
}
...
if ( ! function_exists( 'wp_login_url' ) ) {
	function wp_login_url( string $redirect = '' ): string {
		return 'http://example.com/wp-login'; }
}
if ( ! function_exists( 'wp_die' ) ) {
	function wp_die( $message = '', string $title = '', $args = array() ): void {
		exit; }
}
