...
|---|---|---|
| `tests.yml` | push/PR to `main`, `develop`, or `test` | Unit tests (PHP 8.1–8.4), integration tests (PHP 8.1–8.4 × WP latest/6.4), PHP lint |
| `build-develop.yml` | push to `develop` | Builds plugin ZIP, deploys PWA to `rs-dev`, builds desktop binaries, updates dev APT repo |
| `build-test.yml` | workflow_dispatch | Builds plugin ZIP, deploys PWA to `rs-test`, builds desktop binaries, updates test APT repo |
...
├── manifest.json
├── icons/
├── versions.json     ← Auto-generated by CI
├── versions-beta.json← Auto-generated by CI (same content)
...
└── v/                ← versioned snapshots (server serves under /v/<version>/)
    ...
    │   │   ├── app.js
    │   │   └── icons/
    │   └── beta/             ← Beta PWA files (same content)
    ├── 2.4.26/
    │   └── ...
    └── ...
```
...
- `docs/app/versions-beta.json` contains identical content to `versions.json`. Auto-generated by CI alongside `versions.json`. Provides separate endpoint for beta channel version checking.
- The `build-release.yml` workflow creates the snapshot, prunes to the last 12, and updates both JSON files on every tag push.
...
**Test server (WP site for development):** `<SSH_USER>@<WP_TEST_SERVER_IP>`, WP root at `/srv/www/wordpress`, WP-CLI at `/usr/local/bin/wp`.
...
**Signing key lives in:** root's GPG keyring on `<PWA_SERVER_IP>`. The private key is not stored in the repo or in CI — it is generated once by `bin/setup-apt-repo.sh` during server provisioning.
