...
class RSA_Analytics {
	 ...
	 */
	public static function period_range( string $period ): array {
		...
		switch ( $period ) {
			...
			case 'thismonth':
				...
				break;
			case 'lastmonth':
				$start = strtotime( date( 'Y-m-01', strtotime( '-1 month', $now ) ) );
				$now   = strtotime( date( 'Y-m-t', strtotime( '-1 month', current_time( 'timestamp' ) ) ) . ' 23:59:59' );
				break;
			default: // Default 30d
				$start = strtotime( '-30 days', $now );
		}
	...
	public static function get_overview( string $period = '30d' ): array {
		global $wpdb;
	...
	public static function get_top_pages( string $period = '30d', int $limit = 20 ): array {
		global $wpdb;
		$range = self::period_range( $period );
		$et    = RSA_DB::events_table();
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				...
				$range['start'], $range['end'], self::bot_threshold(), $limit
			),
			ARRAY_A
		);
	...
	public static function get_audience( string $period = '30d' ): array {
		global $wpdb;
		$range = self::period_range( $period );
		$et    = RSA_DB::events_table();
	...
	public static function get_referrers( string $period = '30d', int $limit = 20 ): array {
		global $wpdb;
		$range = self::period_range( $period );
		$et    = RSA_DB::events_table();
		...
		return array_map( fn( $r ) => [ 'domain' => $r['domain'], 'visits' => (int) $r['visits'] ], $rows );
	}
	...
	public static function get_behavior( string $period = '30d' ): array {
		global $wpdb;
		$range = self::period_range( $period );
		$et    = RSA_DB::events_table();
		$st    = RSA_DB::sessions_table();
		...
		$histogram_rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT
					...
					COUNT(*) AS count
				 FROM `{$et}`
				 WHERE created_at BETWEEN %s AND %s AND bot_score < %d AND time_on_page > 0
	...
	public static function get_click_map( string $period = '30d', string $page = '' ): array {
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT element_tag, element_id, element_class, href_protocol, COUNT(*) AS clicks
				 FROM `{$ct}`
				 WHERE created_at BETWEEN %s AND %s {$page_clause}
				 GROUP BY element_tag, element_id, element_class, href_protocol
